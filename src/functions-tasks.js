...
function getPowerFunction(exponent) {
  const exponenta = exponent;
  return (value) => value ** exponenta;
}
...
function getPolynom(...args) {
  ...
  return (num) => {
    return args.reduce(
      (sum, curr, index) => sum + curr * num ** (lastIndex - index),
      0
    );
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argsString = [];
    args.forEach((arg) => {
      argsString.push(JSON.stringify(arg));
    });
    logFunc(`${func.name}(${argsString.join(',')}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argsString.join(',')}) ends`);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return function partial(...args2) {
    const argument = [...args1, ...args2];
    if (argument.length >= fn.length) return fn(...argument);
    return partialUsingArguments(fn, ...argument);
  };
}
